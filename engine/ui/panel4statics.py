...
if __name__ == "__main__":
    ...
    ms = []
    argv = sys.argv
    if 0 and ('doc' in argv or 'all' in argv):
        from reporter.engine.basedb.base import Document
        d = Document()

        d.ime.ime = 'ime'
        d.header.docnum = 1234
        d.viewTemplate = Document.ViewTemplate()
        d.viewTemplate.description = 'xy and z'
        ms.append(d)

    if 'trud' in argv or 'all' in argv:
#        from reporter.engine.basedb.types_base import SubStruct
#        SubStruct.auto_set = True

        from reporter.engine.basedb.docs import TrudovDogovor, Person, Passport
        d = TrudovDogovor()
        d.sluzhitel = Person()
        d.sluzhitel.ime.ime = 'Pebcho'
        d.sluzhitel.egn = 'alabal'
        d.sluzhitel.passport = Passport()
        d.sluzhitel.passport.number = 'pasno'
        d.sluzhitel.passport.izdatel= 'rdvr'
        d.osnovna_zaplata = 3345.5
        #e.staz =
        ms.append(d)
